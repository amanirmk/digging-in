...
for (resolution in c("all", "comma", "object")) {
  for (critonward in c(FALSE, TRUE)) {
    ...
    run_brms(
      ...
      data = brm_data,
      file_path = here("analysis_outputs", "npz_spr_stats", "predicted", paste0("rt_ambxlenxfin_", resolution, if (critonward) "_critonward" else "", ".txt"))
    )
  }
}
    ...
    for (critonward in c(FALSE, TRUE)) {
      for (critoffset in c(0, 1)) {
        ...
        run_brms(
          ...
          data = brm_data,
          file_path = here("analysis_outputs", "npz_spr_stats", "predicted", paste0("rt_ambxlen_", finality, "_", resolution, if (critonward) "_critonward" else "", if (critoffset == 1) "_spillover" else "", ".txt"))
        )
}}}}
...
for (finality in c("final", "nonfinal")) {
  ...
  run_brms(
    ...
    data = brm_data,
    file_path = here("analysis_outputs", "npz_spr_stats", "predicted", paste0("rt_resxlen_", finality, ".txt"))
  )
}