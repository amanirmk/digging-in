    ...
    for (critonward in c(FALSE, TRUE)) {
      for (critoffset in c(0, 1)) {
        ...
        run_brms(
          ...
          data = brm_data,
          file_path = here("analysis_outputs", "npz_maze_stats", "predicted", paste0("rt_ambxlen_", finality, "_", resolution, if (critonward) "_critonward" else "", if (critoffset == 1) "_spillover" else "", ".txt"))
        )
}}}}