...
# Load data
df <- read_csv(here("collected_data", "npz_maze", "human", "unfiltered.csv"))

# Filter for participant accuracy on fillers (excluding first word)
filler_accuracy <- df %>%
  filter(item_category == "filler", word_index > 0) %>%
...
write_csv(filtered, here("collected_data", "npz_maze", "human", "filtered.csv"))