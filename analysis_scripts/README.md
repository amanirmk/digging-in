...
*Please note that the code in these pipelines is not fully polished and that you may see various warnings while executing it (e.g., deprecation, NANs produced, etc). These warnings should not be cause for concern and can be ignored. If you determine that there is a meaningful result-changing mistake on our part, please let us know.*
...
Generate all figures for a given experiment. These run very fast on a personal computer. Note that figures may not be the same as those used in published materials.
```
